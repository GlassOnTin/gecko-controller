                    ...
                    function initializeDisplayImage() {
                        console.log('Initializing display image view');
                        
                        // Create or get container
                        let container = document.getElementById('liveDisplay');
                        if (!container) {
                            console.log('Creating new display container');
                            container = document.createElement('div');
                            container.id = 'liveDisplay';
                        }
                        // Always update container styles
                        container.className = 'mx-auto p-4';
                        container.style.width = '200px';  // Slightly wider than display for padding
                    
                        // Get or create display wrapper and update its styles
                        let displayWrapper = container.querySelector('.display-wrapper');
                        if (!displayWrapper) {
                            displayWrapper = document.createElement('div');
                            container.appendChild(displayWrapper);
                        }
                        // Always update wrapper styles
                        displayWrapper.className = 'bg-white rounded-lg p-4 flex items-center justify-center display-wrapper';
                        displayWrapper.style.width = '171px';  // 512/3 = 171px
                        displayWrapper.style.height = '85px';   // 256/3 ≈ 85px
                        displayWrapper.style.margin = '0 auto';
                    
                        // Get or create image and update its styles
                        let img = container.querySelector('#displayImage');
                        if (!img) {
                            img = document.createElement('img');
                            img.id = 'displayImage';
                            displayWrapper.appendChild(img);
                        }
                        // Always update image styles
                        img.alt = 'OLED Display';
                        img.style.width = '100%';
                        img.style.height = '100%';
                        img.style.imageRendering = 'pixelated';
                        img.style.filter = 'invert(1)';
                        img.style.display = 'none';  // Start hidden until we have an image
                    
                        // Make sure container is in the document
                        if (!container.parentNode) {
                            const statusContainer = document.getElementById('statusContainer');
                            if (statusContainer && statusContainer.parentNode) {
                                statusContainer.parentNode.insertBefore(container, statusContainer.nextSibling);
                            } else {
                                document.body.appendChild(container);
                            }
                        }
                        ...
                        // Start display updates with 10s interval
                        console.log('Starting display updates (10s interval)');
                        updateDisplayImage();  // Initial update
                        displayUpdateInterval = setInterval(updateDisplayImage, 10000);
                    }

                    async function updateDisplayImage() {
                        const img = document.getElementById('displayImage');
                        if (!img) {
                            console.warn('Display image element not found');
                            return;
                        }
                    
                        try {
                            console.log('Fetching display update');
                            const response = await fetch('/api/display');
                            if (!response.ok) {
                                if (response.status === 404) {
                                    console.warn('Display image not yet available');
                            ...
                            if (data.status === 'success' && data.image) {
                                console.log('Received new display image');
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        // Initialize live view of OLED display
                        initializeDisplayImage();
                        console.log('Display initialization requested');
                        ...
                        try {
                            ...
                            // Every 10s
                            setInterval(updateDisplayImage, 10000);
